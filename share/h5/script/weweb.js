...
function onNavigate(url) {
  ...
  if (type == 'reLaunch' && util.isTabbar(curr.path) && (0, _keys2.default)(curr.query).length > 0) {
  ...
  window.exparser.registerElement({
    ...
    _inputKey: function _inputKey(event) {
      ...
      if (this.bindinput) {
        ...
        H5WeixinJSBridge.publish('SPECIAL_PAGE_EVENT', {
...
function showPickerView(args) {
  ...
  picker.on('select', function (n) {
    H5WeixinJSBridge.subscribeHandler('showPickerView', {
...
function insertHTMLWebView(args) {
  ...
  H5WeixinJSBridge.subscribeHandler('insertHTMLWebView', {
...
function showDatePickerView(args) {
  ...
  picker.on('select', function (val) {
    H5WeixinJSBridge.subscribeHandler('showDatePickerView', {
    ...
    value: function postMessage(data) {
      this.onReady(function () {
        ...
        if (command === 'MSG_FROM_APPSERVICE') {
          H5WeixinJSBridge.subscribeHandler(msg.eventName, msg.data);
        } else if (command == 'GET_JSSDK_RES' || command == 'INVOKE_SDK' || /^private_/.test(msg.sdkName)) {
          H5WeixinJSBridge.subscribeHandler(msg.sdkName, msg.res, ext); // ext其实也没用 了
        } else if (command === 'STOP_PULL_DOWN_REFRESH') {
          H5WeixinJSBridge.pull.reset();
        }
      });
    }
  }, {
  ...
  (0, _createClass3.default)(PageParser, [{
    ...
    value: function setData(dataObj, callback) {
      try {
        ...
        if (window.reRender) {
          H5WeixinJSBridge.subscribeHandler('custom_event_appDataChange', {
            data: {
              data: dataObj
            }
          });
...
window.H5WeixinJSBridge = {
  ...
  H5WeixinJSBridge.publish.apply(H5WeixinJSBridge, params);
}
...
function subscribe() {
  ...
  H5WeixinJSBridge.subscribe.apply(H5WeixinJSBridge, params);
}
...
// 订阅并转发 H5WeixinJSBridge 提供的全局事件到 exparser
;(function (glob) {
  H5WeixinJSBridge.subscribe('onAppRouteDone', function () {
  ...
  H5WeixinJSBridge.subscribe('setKeyboardValue', function (event) {
  ...
  H5WeixinJSBridge.subscribe('hideKeyboard', function (e) {
    exparser.triggerEvent(document, 'hideKeyboard', {}, {
      bubbles: !0
    });
  });
  H5WeixinJSBridge.on('onKeyboardComplete', function (event) {
  ...
  H5WeixinJSBridge.on('onKeyboardConfirm', function (event) {
  ...
  H5WeixinJSBridge.on('onTextAreaHeightChange', function (event) {
  ...
  H5WeixinJSBridge.on('onKeyboardShow', function (event) {
...
;(function (window) {
  ...
  var analyticsConfig = {},
    ...
    for (var i = 0; i < analyticsConfig.data.length; i++) {
      ...
      if (curData.element === selector) {
        ...
        H5WeixinJSBridge.publish('analyticsReport', {
          data: data
        });
        break;
      }
  ...
  H5WeixinJSBridge.subscribe('analyticsConfig', function (params) {
...
window.exparser.registerElement({
  ...
  srcChange: function srcChange(e, t) {
    if (this._isReady) {
      var n = this.uuid;
      H5WeixinJSBridge.invoke('updateHTMLWebView', { htmlId: n, src: (e || '').trim() }, function (e) {});
  ...
  attached: function attached() {
    ...
    wx.getSystemInfo({
      success: function success(e) {
        ...
        H5WeixinJSBridge.invoke('insertHTMLWebView', {
          htmlId: n,
          position: {
            left: 0,
            top: 0,
  ...
  detached: function detached() {
    ...
    var t = this.uuid;
    H5WeixinJSBridge.invoke('removeHTMLWebView', { htmlId: t }, function (t) {
