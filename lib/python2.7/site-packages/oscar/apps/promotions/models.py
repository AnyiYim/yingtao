...
class AbstractPromotion(models.Model):
    ...
    display = models.CharField(default='hide', max_length=10)

    sort_num = models.PositiveIntegerField(_('sort of banner'), unique=True, blank=True, null=True)
...
class AutomaticProductList(AbstractProductList):
    ...
    def get_queryset(self):
        # filter by category
        if self.category:
            qs = self.category.product_set.all().select_related('stats')
        else:
