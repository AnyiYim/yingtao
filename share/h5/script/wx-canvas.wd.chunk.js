...
exports.default = window.exparser.registerElement({
  ...
  attached: function attached() {
    ...
    if (window.__canvasNumbers__.hasOwnProperty(canvasId)) {
      ...
      H5WeixinJSBridge.publish('canvasInsert', {
        canvasId: self.canvasId,
        canvasNumber: self._canvasNumber
      });
      H5WeixinJSBridge.subscribe('canvas' + self._canvasNumber + 'actionsChanged', function (params) {
        var actions = params.actions,
            reserve = params.reserve;
        self.actions = actions;
        self.actionsChanged(actions, reserve);
      });
      H5WeixinJSBridge.subscribe('invokeCanvasToDataUrl_' + self._canvasNumber, function () {
        var dataUrl = self.$.canvas.toDataURL();
        H5WeixinJSBridge.publish('onCanvasToDataUrl_' + self._canvasNumber, {
          dataUrl: dataUrl
        });
      });
      self._ready();
  ...
  detached: function detached() {
    ...
    H5WeixinJSBridge.publish('canvasRemove', {
