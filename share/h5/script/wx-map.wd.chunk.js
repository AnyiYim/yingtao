...
window.exparser.registerElement({
  ...
  _update: function _update(opt, t) {
    ;opt.mapId = this._mapId, opt.hide = this.hidden, H5WeixinJSBridge.invoke('updateMap', opt, function (e) {});
  },
  ...
  _initFeatures: function _initFeatures() {
    this._mapId && ((this.markers && this.markers.length > 0 || this.covers && this.covers.length > 0) && H5WeixinJSBridge.invoke('addMapMarkers', {
      mapId: this._mapId,
      markers: this._transformMarkers(this.markers).concat(this.covers)
    }, function (e) {}), this.includePoints && this.includePoints.length > 0 && H5WeixinJSBridge.invoke('includeMapPoints', {
      mapId: this._mapId,
      points: this.includePoints
    }, function (e) {}), this.polyline && this.polyline.length > 0 && H5WeixinJSBridge.invoke('addMapLines', {
      mapId: this._mapId,
      lines: this.polyline
    }, function (e) {}), this.circles && this.circles.length > 0 && H5WeixinJSBridge.invoke('addMapCircles', {
      mapId: this._mapId,
      circles: this.circles
    }, function (e) {}), this.controls && this.controls.length > 0 && H5WeixinJSBridge.invoke('addMapControls', {
      mapId: this._mapId,
  ...
  _insertIframeMap: function _insertIframeMap() {
    ...
    var mapTitlesLoadedEvent = qq.maps.event.addListener(map, 'tilesloaded', function () {
      ...
      H5WeixinJSBridge.subscribe('doMapAction' + self._mapId, function (res) {
        if (self._map && self._mapId === res.data.mapId) {
          if (res.data.method === 'getMapCenterLocation') {
            ...
            res.data.method === 'moveToMapLocation' && self.showLocation && H5WeixinJSBridge.invoke('private_geolocation', {}, function (res) {
              try {
                res = JSON.parse(res);
              } catch (e) {
                res = {};
      ...
      H5WeixinJSBridge.publish('mapInsert', {
        domId: self.id,
        mapId: self._mapId,
  ...
  showLocationChanged: function showLocationChanged(location, uselessArg) {
    var self = this;
    this._isReady ? (this._posOverlay && (this._posOverlay.setMap(null), this._posOverlay = null), location && H5WeixinJSBridge.invoke('private_geolocation', {}, function (res) {
      try {
        res = JSON.parse(res);
      } catch (e) {
        res = {};
